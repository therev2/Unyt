...
export default function DialogicModePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/learn-ai", {
        ...
        body: JSON.stringify({ mode: "dialogic-mode", topic }),
      });
      const data = await res.json();
      const parsed = JSON.parse(data.result);
      setDialogue(parsed);
    } catch (err) {
      setError("Failed to generate dialogic mode. Try again.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-8 bg-background">
      <h1 className="text-3xl font-bold mb-4 text-center">Dialogic Mode</h1>
      <p className="text-lg text-muted-foreground mb-8 text-center max-w-xl">
        Enter a topic and engage in an interactive AI-powered conversation to learn.
      </p>
      ...
      <div className="mt-8 w-full max-w-2xl space-y-8">
        {dialogue.map((turn, i) => (
          <div key={i} style={cardStyle} className="flex flex-col items-center justify-center">
            <div className="font-semibold mb-3 text-lg">{turn.speaker}:</div>
            <div className="text-muted-foreground text-base">{turn.text}</div>
          </div>
        ))}
      </div>
    </div>
